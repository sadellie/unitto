...
interface TextFieldStateTokenExtensions {
  val closeBeforeOperators: List<String>
  val openAfterOperators: List<String>
  val longTokens: List<String>
  ...
  fun TextFieldState.addBracket() {
    ...
    // Always close before some operators
    if (text.toString().tokenAfter(selection.min, longTokens) in closeBeforeOperators) {
    ...
    // Always open after some operators
    if (text.toString().tokenAhead(selection.min, longTokens) in openAfterOperators) {
      addTokens(Token.LeftBracket.symbol)
      return
    }
  ...
  fun TextFieldState.deleteTokens() {
    val deleteRangeStart =
      when (selection.max) {
        ...
        selection.min -> selection.min - text.toString().tokenLengthAhead(selection.max, longTokens)
  ...
  fun TextFieldState.deleteAheadAndAdd(tokens: String) {
