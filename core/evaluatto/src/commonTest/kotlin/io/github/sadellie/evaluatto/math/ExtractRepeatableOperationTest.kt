...
class ExtractRepeatableOperationTest {
  private val cx = ScriptContext.Math(2)

  @Test
  fun simpleValid() {
    assertExtractedOperation("1+2", Operation.Plus("2"))
    assertExtractedOperation("1−2", Operation.Minus("2"))
    assertExtractedOperation("1×2", Operation.Multiply("2"))
    assertExtractedOperation("1÷2", Operation.Divide("2"))
  }
  ...
  private fun assertExtractedOperation(input: String, expected: Operation?) {
    ...
    val tree = ASTMathBuilder(tokens).buildTreeAndCollapse(cx) ?: error("No trees")
