...
internal data class BracketsNode(override val children: List<ASTNode>) : ASTNode {
  override val token: Token
    get() = error("BracketsNode has no token")
  ...
  override fun collapse(scriptContext: ScriptContext) =
    children.firstOrNull()?.collapse(scriptContext) ?: super.collapse(scriptContext)
}
