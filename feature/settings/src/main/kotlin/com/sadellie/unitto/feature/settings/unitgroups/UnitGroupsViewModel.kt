...
constructor(private val userPrefsRepository: UserPreferencesRepository) : ViewModel() {
  ...
  val uiState =
    userPrefsRepository.unitGroupsPrefs
      .map {
  ...
  fun removeShownUnitGroup(unitGroup: UnitGroup) {
    viewModelScope.launch { userPrefsRepository.removeShownUnitGroup(unitGroup) }
  }

  /** @see UserPreferencesRepository.addShownUnitGroup */
  fun addShownUnitGroup(unitGroup: UnitGroup) {
    viewModelScope.launch { userPrefsRepository.addShownUnitGroup(unitGroup) }
  }

  /** @see UserPreferencesRepository.updateShownUnitGroups */
  fun updateShownUnitGroups(unitGroups: List<UnitGroup>) {
    viewModelScope.launch { userPrefsRepository.updateShownUnitGroups(unitGroups) }
  }
}
